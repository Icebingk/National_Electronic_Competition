...
mag=[1 0];       %窗函数的理想滤波器幅度
dev=[0.01 0.05]; %纹波
[n,wn,beta,ftype]=kaiserord(fc,mag,dev,fs);  %获取凯塞窗参数，分别是滤波器长度、截止频率、窗函数的beta值和窗函数类型
fpm=[0 fc(1)*2/fs fc(2)*2/fs 1];  %firpm函数的频段向量，归一化了
...
%求滤波器的幅频响应，傅里叶变换+FFT函数的长度为1024+取绝对值+对数变换
%幅频响应的单位为d
