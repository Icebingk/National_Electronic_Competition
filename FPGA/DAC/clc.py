...
def print_result(params: Dict[str, Any], sys_clock: float, target_freq: float) -> None:
    """漂亮地打印计算结果"""
    print("\n" + "="*50)
    print(f"【DDS参数计算结果】")
    print("="*50)
...
def interactive_mode() -> None:
    """交互式计算模式"""
    print("\n" + "="*60)
    print("     DDS频率参数计算工具 v2.0")
    print("     用于FPGA DDS设计的频率控制参数计算")
    print("="*60)
    ...
    while True:
        try:
            ...
            # 生成Verilog代码
            print("\n-- Verilog定义代码 --")
            target_mhz = int(target_freq/1_000_000)
            clock_mhz = int(sys_clock/1_000_000)
            
            print(f"`define FREQ_CTRL_MHZ {format_verilog(params['fcw'])} " + 
                  f"// {target_freq/1_000_000:.2f}MHz@{sys_clock/1_000_000:.2f}MHz时钟")
            
            if step_params:
                step_mhz = int(step_freq/1_000_000)
                print(f"`define FREQ_STEP_MHZ {format_verilog(step_params['fcw'])}  " + 
            ...
            if max_input and max_input.lower() not in ('q', 'quit', 'exit'):
                ...
                print(f"`define FREQ_MAX_LIMIT {format_verilog(max_params['fcw'])} " + 
