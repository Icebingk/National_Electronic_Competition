...
// 第二路波形 (kHz级)
`define FREQ_CTRL_2_KHZ 32'd34359 // 1.00kHz@125.00MHz时钟
`define FREQ_STEP_2_KHZ 32'd34359  // 1.00kHz步进@125.00MHz时钟
`define FREQ_MAX_2_LIMIT 32'd343597 // 10.00kHz最大值
// 波形选择编码
...
module dac_generator_integrated (
    ...
    reg   [2*`DAC_BITS-1:0] am_out_reg;   // am输出寄存器
    wire                 pll_locked;     // PLL锁定状态指示1
    
    // 系统复位信号 (PLL锁定后才释放) - 高电平复位
    assign dac_clk1 = clk; // DAC时钟输出直接连接到系统时钟
    assign dac_clk2 = clk; // DAC时钟输出直接连接到系统时钟
    ...
    // 相位地址输出 (截取高位用于波形查找)
    assign phase_addr1 = phase_acc1[31:19]; // 13位相位地址
    assign phase_addr2 = phase_acc2[31:19]; // 13位相位地址
    ...
    always @(posedge clk or negedge rst) begin
        if (!rst) begin
            ...
            // 根据频率控制字调整振幅补偿 - 精细分段
            // 频率越高，补偿系数越大
            if (freq_ctrl1 < 32'd171798692)           // < 5MHz
                amplitude_comp1 <= 5'd10;         // 1.0x
            else if (freq_ctrl1 < 32'd343597383)      // 5-10MHz
            ...
            else if (freq_ctrl1 < 32'd1546188226)     // 40-45MHz
                amplitude_comp1 <= 5'd18;         // 1.8x
            else if (freq_ctrl1 < 32'd1717986918)     // 45-50MHz
                amplitude_comp1 <= 5'd19;         // 1.9x
            else                                     // > 50MHz
                amplitude_comp1 <= 5'd20;         // 2.0x
        end
    end
    ...
    always @(posedge clk or negedge rst) begin
        if (!rst) begin
            ...
            // 根据频率控制字调整振幅补偿 - 为kHz级频率精细分段
            // 频率越高，补偿系数越大
            if (freq_ctrl2 < 32'd34359)              // < 1kHz
                amplitude_comp2 <= 5'd10;         // 1.0x
            else if (freq_ctrl2 < 32'd68718)         // 1-2kHz
                amplitude_comp2 <= 5'd11;         // 1.1x
            else if (freq_ctrl2 < 32'd103077)        // 2-3kHz
                amplitude_comp2 <= 5'd12;         // 1.2x
            else if (freq_ctrl2 < 32'd137436)        // 3-4kHz
                amplitude_comp2 <= 5'd13;         // 1.3x
            else if (freq_ctrl2 < 32'd171795)        // 4-5kHz
                amplitude_comp2 <= 5'd14;         // 1.4x
            else if (freq_ctrl2 < 32'd206154)        // 5-6kHz
                amplitude_comp2 <= 5'd15;         // 1.5x
            else if (freq_ctrl2 < 32'd240513)        // 6-7kHz
                amplitude_comp2 <= 5'd16;         // 1.6x
            else if (freq_ctrl2 < 32'd274872)        // 7-8kHz
                amplitude_comp2 <= 5'd17;         // 1.7x
            else if (freq_ctrl2 < 32'd309231)        // 8-9kHz
                amplitude_comp2 <= 5'd18;         // 1.8x
            else if (freq_ctrl2 < 32'd343590)        // 9-10kHz
                amplitude_comp2 <= 5'd19;         // 1.9x
            else                                     // > 10kHz
                amplitude_comp2 <= 5'd20;         // 2.0x
        end
    end
    // 应用振幅补偿
    wire [32:0] sin_comp_mult1 = sin_data1 * amplitude_comp1;
    wire [13:0] sin_compensated1 = (sin_comp_mult1 / 10) > 14'h3FFF ? 14'h3FFF : sin_comp_mult1 / 10; // 限制在14位范围内

    wire [32:0] sin_comp_mult2 = sin_data2 * amplitude_comp2;
    wire [13:0] sin_compensated2 = (sin_comp_mult2 / 10) > 14'h3FFF ? 14'h3FFF : sin_comp_mult2 / 10; // 限制在14位范围内
    ...
    always @(posedge clk or negedge rst) begin
        if (!rst) begin
            ...
            case (wave_sel1)
                3'b110 : dac_data_reg1 <= sin_compensated1;
                3'b101 : dac_data_reg1 <= tri_data1;
                3'b011 : dac_data_reg1 <= sqr_data1;
                default : dac_data_reg1 <= sin_compensated1;
            endcase
        end
    end
    always @(posedge clk or negedge rst) begin
        if (!rst) begin
            ...
            case (wave_sel2)
                3'b110 : dac_data_reg2 <= sin_compensated2;
                3'b101 : dac_data_reg2 <= tri_data2;
                3'b011 : dac_data_reg2 <= sqr_data2;
                default : dac_data_reg2 <= sin_compensated2;
            endcase
        end
    end
    //===================================================
    ...
    // AM调制输出寄存器
    //===================================================
    wire [27:0] am_limited = (am_out_wire[2*`DAC_BITS-1:`DAC_BITS] > 14'h3FFF) ? 14'h3FFF : am_out_wire[2*`DAC_BITS-1:`DAC_BITS];
    always @(posedge clk or negedge rst) begin
        if (!rst) begin
            am_out_reg <= 14'd0;
        end else begin
            // AM调制输出 - 使用乘法器计算
            am_out_reg <= am_limited; // 直接使用14位数据
        end
    end
    assign am_out = am_out_reg;
    //===================================================
    // PLL模块内联
    //===================================================
    // 原PLLM模块功能集成
