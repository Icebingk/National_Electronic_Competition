/*==============================================
* Function Name  : buzz.v
* Description    : 该模块内部生成可变频率的方波信号，
*                  通过选择不同的频率预设值或自定义分频值来控制蜂鸣器的发声频率。
* input port     : clk(系统时钟), rst_n(复位信号), en(使能信号),
*                  freq_sel(频率选择), custom_div(自定义分频值)
* output port    : buz(蜂鸣器输出信号) 
* Author         : ADBD
//==============================================*/
module buzz(
    input       wire            clk,            // 系统时钟
