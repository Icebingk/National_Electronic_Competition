module ADC(
    ...
    input wire          adc_clkx,         // ADC时钟

    input wire          OTR,            // 超量程提示
    input wire [11:0]   adc_data_in,  // ADC数据输入

    output reg [11:0]   adc_data_out,  // ADC数据输出
...
assign adc_data_out = adc_data; // 将FIFO输出数据赋值给adc_data_out

assign wr_en = !full & !OTR; // 当FIFO未满且未超量程时允许写入
assign rd_en = !empty; // 当FIFO不为空时允许读取
assign adc_data_valid = !empty; // 当FIFO不为空时数据有效
